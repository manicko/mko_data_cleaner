...
from mko_data_cleaner.core.models import DataSettings, LoggingSettings, WorkingPaths
...
class AppService:
    ...
    @cached_property
    def data_paths(self) -> WorkingPaths:
        return self.app_config.data_paths

    @cached_property
    def base(self) -> Path:
        return self.resolver.ensure_dir(
            self.resolver.resolve(
                self.data_paths.base_folder
            )
        )

    @cached_property
    def import_path(self) -> Path:
        return self.base /self.data_paths.import_folder

    @cached_property
    def export_path(self) -> Path:
        return self.base / self.data_paths.export_folder

    @cached_property
    def dict_path(self) -> Path:
        return self.base / self.data_paths.dict_folder

    @cached_property
    def dict_file(self) -> Path:
        return self.dict_path / self.data_paths.dict_file_name

    @property
    def db_file(self) -> Path:
        return self.base / self.data_paths.db_folder / self.data_paths.db_file_name
    ...
    def run_report(self):
        ...
        print(self.db_file, self.import_path, flush=True)
...
if __name__ == "__main__":
    print(app_service.log_config.model_dump())
    app_service.run_report()
